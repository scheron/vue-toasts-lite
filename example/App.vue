...
const toastListCount = ref(0)
const unsubscribeToastList = ref<(() => void) | null>(null)

const startMonitoring = () => {
  toastListCount.value = toasts.toastList.length

  unsubscribeToastList.value = toasts.onToastsListChange((toastsList) => {
    toastListCount.value = toastsList.length
  })
}

const stopMonitoring = () => {
  if (unsubscribeToastList.value) {
    unsubscribeToastList.value()
    unsubscribeToastList.value = null
  }
}

const showToastListDemo = () => {
  toasts.success("It works!")
  toasts.error("Error!")
}
...
onMounted(() => {
  ...
  startMonitoring() // Start monitoring toast list changes
})

onUnmounted(() => {
  window.removeEventListener("scroll", updateActiveSection)
  stopMonitoring() // Clean up subscription
})
  ...
  <div class="docs-wrapper">
    ...
    <aside class="sidebar">
      <nav class="sidebar-nav">
        ...
        <a href="#toast-list" :class="['nav-item', 'nav-subitem', {'nav-active': activeSection === 'toast-list'}]">Toast List & Monitoring</a>
...
toasts.onToastsListChange(callback) // Subscribe to toast list changes</code></pre>
      </section>
      ...
      <section id="options" class="section">
        ...
        <pre
          class="code-block"
        ><code class="language-javascript">type ToastPosition = "top-left" | "top-center" | "top-right" | "bottom-left" | "bottom-center" | "bottom-right" | "middle-center"</code></pre>
      </section>
...
toasts.clear()</code></pre>
        ...
        <h3 id="toast-list">Toast List & Monitoring</h3>
        <pre class="code-block"><code class="language-javascript">// Get current toast list
const currentToasts = toasts.toastList
console.log('Active toasts:', currentToasts)
...
const unsubscribe = toasts.onToastsListChange((toastsList) => {
  ...
  console.log('Total toasts:', toastsList.length)
})
...
unsubscribe()</code></pre>
        ...
        <div class="demo-buttons" style="flex-direction: column; gap: 0.75rem">
          <div style="display: flex; gap: 0.75rem; width: 100%">
            <button class="btn btn-primary" @click="showToastListDemo" style="flex: 1">Show Current Toast Count</button>
          </div>
          <div style="padding: 0.75rem; background: #f5f5f5; border-radius: 4px; border: 1px solid #e5e5e5">
            <p style="margin: 0; color: #666; font-size: 0.875rem">
              Active toasts: <strong style="color: #1a1a1a">{{ toastListCount }}</strong>
              <span style="color: #999; font-size: 0.8125rem"></span>
            </p>
          </div>
        </div>
      </section>
